...
<script setup>
    ...
    onMounted(async () => {
        // 发送请求
        const userListData = await myAxios.get('/user/search/tagsOr', {
            //这里的await的作用是:执行完get方法后,才能得到userListData
            params: {
                tagList: activeIds
            },
            //用鱼皮的这个我的头像不会显示。
            ...
            .then(function (response) {
                console.log("/user/search/tagsOr  success", response);
                console.log(response.data);
                showSuccessToast("请求成功");
                return response?.data;//这里的？号作用是：当response.data为null时，不再取后面的.data
            })
            .catch(function (error) {
                console.log("/user/search/tagsOr fail", error);
                showFailToast("请求失败")
            });
    ...
    /*const userList = ref([{
        ...
        phone: "12345678901",
        email: "xx@qq.com",
        profile: "自学编程两年半,擅长唱跳rapper,阿巴阿巴阿巴巴巴巴巴阿巴",
        userRole: "管理员",
        tags: ["java", "emo", "打工人"],
        createTime: new Date()
    }])*/
